    ...
    for (i=0; i<n; i++) {
        for (j=0; j<n; j++) {
            for (k=0; k<n; k++) {
                ...
                /*. This ensures that each thread obtains a unique value of p during each iteration
                of the loop without interference from other threads*/
                current_p = p++;
                if (current_p<N) {
                    
                    r[current_p][0] = (i + 0.5)*pos;
                    r[current_p][1] = (j + 0.5)*pos;
                    r[current_p][2] = (k + 0.5)*pos;
                }
            }
        }
    }
